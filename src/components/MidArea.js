...
import ActionInput from './inputaction.js';
        ...
        {selectedSprite ? (
          <>
            <div className='flex gap-4'>
              <h2 className="text-2xl flex font-bold text-gray-800 mb-4">Selected Sprite Action
              </h2>
              <div className='text-sm'><SpriteImage spriteName={selectedSprite.name} styles={{ width: "50px", height: "50px" }} /></div>
            </div>
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-700 mb-2">Current Sprite Actions:</h3>
              <ul className="space-y-2">
                {selectedSprite.actions.map((action, index) => (
                  <li key={index} className="bg-blue-100 text-blue-800 px-3 py-1 flex justify-between align-middle text-sm">
